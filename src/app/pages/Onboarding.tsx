  ...
  if (step === 5) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-complexo-dark p-5 text-center">
        ...
        <h1 className="mt-8 font-rajdhani text-5xl font-bold uppercase text-white">Bem-vindo ao Universo Complexo</h1>
        ...
        <div className="mt-10 rounded-3xl border border-white/10 bg-complexo-surface p-8">
          ...
          <p className="mt-4 font-mono text-2xl font-bold tracking-widest text-white">CPLX-8492</p>
        </div>

        <button
          onClick={() => navigate("/dashboard/aluno")}
          className="mt-10 inline-flex items-center gap-2 rounded-full bg-white px-8 py-4 font-semibold text-black transition-transform hover:scale-105"
        >
        ...
        <div className="mb-12 flex items-center justify-between gap-2">
          {STEPS.map((s, i) => (
            <div key={s} className="flex-1">
              <div className={`h-1.5 w-full rounded-full transition-colors ${i <= step ? "bg-complexo-red" : "bg-white/10"}`} />
              <p className={`mt-2 hidden text-xs font-semibold uppercase tracking-wider sm:block ${i <= step ? "text-white" : "text-complexo-muted"}`}>
                {s}
              </p>
            </div>
          ))}
        ...
        <div className="max-w-xl w-full mx-auto">
          {step === 0 && (
            <div className="animate-in fade-in slide-in-from-right-4">
              ...
              <h2 className="font-rajdhani text-3xl font-bold uppercase text-white">Dados Pessoais</h2>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <select className="w-full rounded-xl border border-white/10 bg-complexo-panel px-4 py-4 text-white placeholder:text-complexo-muted focus:border-complexo-red focus:outline-none appearance-none">
          ...
          {step === 1 && (
            <div className="animate-in fade-in slide-in-from-right-4">
              ...
              <h2 className="font-rajdhani text-3xl font-bold uppercase text-white">Seus Objetivos</h2>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                {["Emagrecimento", "Hipertrofia", "Condicionamento", "Saúde e Bem-estar", "Performance", "Recuperação"].map(obj => (
                  <label key={obj} className="flex cursor-pointer items-center gap-3 rounded-xl border border-white/10 bg-complexo-panel p-4 transition-colors hover:bg-white/5 has-[:checked]:border-complexo-red has-[:checked]:bg-complexo-red/5">
                    <input type="checkbox" className="h-5 w-5 accent-complexo-red bg-transparent border-white/20 rounded" />
                    <span className="font-semibold text-white">{obj}</span>
                  </label>
                ))}
              </div>
            </div>
          ...
          {step === 2 && (
            <div className="animate-in fade-in slide-in-from-right-4">
              ...
              <h2 className="font-rajdhani text-3xl font-bold uppercase text-white">Informações Físicas</h2>
              ...
              <div className="space-y-4">
                ...
                <textarea 
                  ...
                  className="w-full rounded-xl border border-white/10 bg-complexo-panel px-4 py-4 text-white placeholder:text-complexo-muted focus:border-complexo-red focus:outline-none min-h-[120px] resize-none"
                />
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="animate-in fade-in slide-in-from-right-4">
              ...
              <h2 className="font-rajdhani text-3xl font-bold uppercase text-white">Escolha seu Plano</h2>
              ...
              <div className="space-y-4">
                {PLANS.map(p => (
                  <button 
                    ...
                    className={`flex w-full items-center justify-between rounded-2xl border p-5 transition-all text-left ${selectedPlan === p.id ? "border-complexo-red bg-complexo-red/10 scale-[1.02]" : "border-white/10 bg-complexo-panel hover:border-white/30"}`}
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="font-rajdhani text-2xl font-bold uppercase text-white">{p.name}</h3>
                    ...
                    <div className="text-right">
                      <p className="font-rajdhani text-2xl font-bold text-white">R$ {p.price}</p>
          ...
          {step === 4 && (
            <div className="animate-in fade-in slide-in-from-right-4">
              ...
              <h2 className="font-rajdhani text-3xl font-bold uppercase text-white">Pagamento</h2>
              <p className="mb-8 text-complexo-muted">Finalize sua assinatura para garantir o acesso.</p>
              
              <div className="rounded-xl border border-white/10 bg-complexo-surface p-5 mb-8 flex justify-between items-center">
                <div>
                  <p className="text-sm font-bold text-complexo-muted uppercase">Resumo</p>
                  <p className="font-bold text-white text-lg">{PLANS.find(p=>p.id===selectedPlan)?.name}</p>
                </div>
              ...
              <div className="space-y-4">
                <div className="relative">
                  ...
                  <input placeholder="Número do cartão" className="w-full rounded-xl border border-white/10 bg-complexo-panel py-4 pl-12 pr-4 text-white placeholder:text-complexo-muted/60 focus:border-complexo-red focus:outline-none" />
                </div>
          ...
          <div className="mt-12 flex items-center justify-between border-t border-white/10 pt-8">
            <button 
              ...
              className="inline-flex items-center gap-2 font-semibold text-complexo-muted hover:text-white"
            >
              <ArrowLeft className="h-4 w-4" /> Voltar
            </button>
            <button 
              onClick={handleNext}
              className="inline-flex items-center gap-2 rounded-full bg-complexo-red px-8 py-4 font-semibold text-white transition-transform hover:scale-105 hover:bg-complexo-red-bright"
            >
...
const Input = ({ className = "", ...props }: React.InputHTMLAttributes<HTMLInputElement>) => (
  <input
    {...props}
    className={`w-full rounded-xl border border-white/10 bg-complexo-panel px-4 py-4 text-white placeholder:text-complexo-muted focus:border-complexo-red focus:outline-none ${className}`}
  />
);
